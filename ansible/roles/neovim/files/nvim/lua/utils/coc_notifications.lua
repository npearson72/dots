...
M.setup = function()
  ...
  local function notify_with_replace(kind, msg, level_str)
    ...
    -- Some notify implementations return a table w/ id, others return
    -- an integer. We store whatever comes back so long as it's not
    -- nil for replacement.
  ...
  local function status_notify()
    ...
    if status == '' or string.len(vim.trim(status)) == 0 then
      return
    end
